...
interface IToken1155Factory {
    function initialize(address _address, address _feeAddress) external;

    function transferOwnership(address newOwner) external;
}

contract ProxyMarketplace1155Factory is Ownable, ReentrancyGuard {
    ...
    function _clone(
        ...
        IToken1155Factory(newProxy).initialize(_pndc, _feeAdress);
        IToken1155Factory(newProxy).transferOwnership(_owner);
