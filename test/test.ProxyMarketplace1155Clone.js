...
const ProxyMarketplace1155Factory = artifacts.require(
  "ProxyMarketplace1155Factory"
);
contract("PNDC_ERC1155", (accounts) => {
  contract("ProxyMarketplace1155Factory", (accounts) => {
    describe("Testing Contract Workflow ", function () {
      it("Testing smart contract safe minting", async () => {
        ...
        result = await instance2.mint(accounts[0], 10, [], "uri", [
          [accounts[3], 500],
        ]);
      ...
      it("Testing ProxyMarketplace1155Factory smart contract initialization ", async () => {
        const instance2 = await PNDC_ERC1155.deployed();
        const clone = await ProxyMarketplace1155Factory.deployed();

        result = await clone._clone(
          instance2.address,
          accounts[0],
          accounts[0]
        );
        ...
        //console.log("clone address", result2[0]);

        assert.equal(await result.logs[0].address, result2);
      });
      ...
      it("Testing smart contract sellNFT function ", async () => {
        ...
        const instance = await TokenFactory1155.at(cloneAddress[0]);
      ...
      it("Testing smart contract BuyNFT function ", async () => {
        ...
        result2 = await instance.BuyNFT(1, 2, {
          from: accounts[1],
          value: 1000,
        });
      ...
      it("Testing smart contract SellNFT_byBid", async () => {
        ...
        await instance.SellNFT_byBid(instance2.address, 0, 500, 2, {
          from: accounts[1],
        });
      ...
      it("Testing the burn function", async () => {
        ...
        result = await instance.mint(accounts[0], 5, [], "uriii", [
          [accounts[3], 500],
        ]);
      ...
      it("Testing the implementation updating", async () => {
        ...
        const oldImplementation = await clone.implementation();

        result = await clone.updateImplementation(accounts[3]);

        assert.equal(accounts[3], await clone.implementation());
      });
    });
  });
});
