...
contract("PNDC_ERC721", (accounts) => {
  contract("ProxyMarketplaceFactory", (accounts) => {
    describe("Testing Contract Workflow ", function () {
      it("Testing TokenFactoryClone smart contract initialization ", async () => {
        ...
        const clone = await ProxyMarketplaceFactory.deployed();

        result = await clone._clone(instance.address, accounts[0], accounts[0]);
        //console.log(result);
      ...
      it("Testing smart contract safe minting", async () => {
        ...
        const instance2 = await TokenFactory.at(cloneAddress[0]);
